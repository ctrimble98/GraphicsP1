...
public class HermiteSpline extends Curve {
    ...
    private void fillTangents() {
        ...
        for (int i = 1; i < points.size() - 1; i++) {
            tangents.add(points.get(i+1).sub(points.get(i - 1)).mult(0.5));
        }
    ...
    private Vector3 getPointOnCurve(double u, int i) {
        ...
        return h00.add(h10).add(h01).add(h11);
    }
    ...
    private void fillCurve() {
        ...
        for (int i = 0; i < points.size() - 1; i++) {
            for (double u = 0; u < 1; u += stepsize) {
                curve.add(getPointOnCurve(u, i));
            }
        }
    }

    // https://stackoverflow.com/questions/1813719/intersection-between-bezier-curve-and-a-line-segment
    public List<Vector3> findIntersection(StraightLine line) {
        ...
        for (int i = 0; i < points.size() - 1; i++) {
            ...
            Vector3 R1 = new Vector3(v.getX() / vMag, v.getY() / vMag, 0);
            ...
            try {
                ...
                for (double t: slnsForT) {
                    if (t >= 0 && t <= 1) {
                        intersections.add(getPointOnCurve(t, i));
                    }
                }
